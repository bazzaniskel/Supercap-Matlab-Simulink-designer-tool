...
try
    ...
    % Smart analysis: Divide profile into 1-minute sections and select top 5
    fprintf('Analyzing power profile for top sections with highest variance...\n');
    
    % Define 1-minute window (600 seconds)
    ...
    if num_sections < 1
        fprintf('Warning: Profile duration (%.1f s) is less than 10 minutes. Using entire profile.\n', total_duration);
        num_top_sections = 1;
    else
        % Prompt user for number of sections to select
        max_sections = min(num_sections, 10); % Limit to reasonable number
        fprintf('\nHow many sections with highest variance do you want to select?\n');
        fprintf('Available range: 1 to %d sections\n', max_sections);
        fprintf('Recommended: 3-5 sections for good coverage\n');
...
feralpi_250725_pp = struct();

% Basic information
feralpi_250725_pp.name = 'Feralpi Power Profile';
feralpi_250725_pp.description = 'Power profile loaded from feralpi.xlsx Excel file';

dt = time_raw(2) - time_raw(1);
time_raw = 0:dt:(length(power_raw)-1)*dt;

% Time vector definition - use the data from Excel
feralpi_250725_pp.time_data = time_raw;

% Power data from Excel
feralpi_250725_pp.power_data = power_raw;

% Calculated parameters
feralpi_250725_pp.max_power = max(abs(feralpi_250725_pp.power_data));
feralpi_250725_pp.duration = feralpi_250725_pp.time_data(end);
feralpi_250725_pp.time_step = feralpi_250725_pp.time_data(2) - feralpi_250725_pp.time_data(1);

% Profile type and units
feralpi_250725_pp.Switch_CurrentOrPower = -1; % -1 = power profile, 1 = current profile
feralpi_250725_pp.units = 'W';

% Optional: Add additional metadata
feralpi_250725_pp.created_by = 'Excel Import';
feralpi_250725_pp.creation_date = datestr(now);
feralpi_250725_pp.notes = sprintf('Power profile imported from %s (Time: %.1f-%.1f s, Power: %.1f-%.1f W)', ...
